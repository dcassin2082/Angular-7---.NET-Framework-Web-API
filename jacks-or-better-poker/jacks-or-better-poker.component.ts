...
import { JacksOrBetterService } from './shared/jacks-or-better.service';
import { JacksOrBetter } from './shared/jacks-or-better.model';
import * as $ from 'jquery';
import { JacksOrBetterPayout } from './shared/jacks-or-better-payout.model';
import { environment } from 'src/environments/environment';

declare var $: any;

@Component({
  selector: 'app-jacks-or-better-poker',
  templateUrl: './jacks-or-better-poker.component.html',
  styleUrls: ['./jacks-or-better-poker.component.css']
})
export class JacksOrBetterPokerComponent implements OnInit {

  constructor(public userService: UserService, public jacksOrBetterService: JacksOrBetterService, private http: HttpClient, private router: Router) { }

  bet: number = 0;
  arr: number[] = [0, 0, 0, 0, 0];
  stageOfGame: string = "deal";
  baseUrl: string = environment.production ?  "http://dcassin5938-001-site1.ctempurl.com/api/jacksorbetter" : "https://localhost:44331/api/jacksorbetter/";
  response: JacksOrBetter;
  paytable: JacksOrBetterPayout[];
  credits: number;
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('token') == null) {
      ...
      this.jacksOrBetterService.getPaytable().subscribe(x => {
        this.response = x as JacksOrBetter;
        this.paytable = this.response.JacksOrBetterPaytable;
  ...
  dealDraw() {
    ...
    if (this.stageOfGame == 'deal') {
      ...
      $.ajax({
        ...
        url: environment.production ?  "http://dcassin5938-001-site1.ctempurl.com/api/jacksorbetter/deal" : "https://localhost:44331/api/jacksorbetter/deal",
        data: {
          bet: this.bet,
      ...
      $.ajax({
        ...
        contentType: "application/json",
        dataType: 'json',
        url: environment.production ?  "http://dcassin5938-001-site1.ctempurl.com/api/jacksorbetter/draw/arr" : "https://localhost:44331/api/jacksorbetter/draw/arr",
        traditional: true,
        data: {
          arr: this.arr,
          bet: this.bet,
